...
import { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, NATIVE_MINT, createAssociatedTokenAccountIdempotentInstruction, createSyncNativeInstruction, createCloseAccountInstruction, getAssociatedTokenAddressSync, } from '@solana/spl-token';
/**
 * Ensure owner's ATA for a mint. Returns the ATA pubkey + optional create ix.
 */
export function ensureAtaIx(payer, owner, mint) {
    const ata = getAssociatedTokenAddressSync(mint, owner, false, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
    const createIx = createAssociatedTokenAccountIdempotentInstruction(payer, ata, owner, mint, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
 ...
 */
export function wrapSolIntoAta(payer, owner, amountLamports) {
 ...
 */
export function unwrapWsolAta(owner) {
    return (ata) => {
        if (!ata)
            return [];
        return [
...
//# sourceMappingURL=tokenAta.js.map
...
export function scaleToUnits(amount: string | number, mint: string) {
  const dec = MINT_DECIMALS[mint];
  if (dec === undefined) throw new Error(`Unknown mint decimals for ${mint}`);
  const [i, f = ""] = String(amount).split(".");
  if (f.length > dec) throw new Error(`Too many decimal places for ${mint} (${dec})`);
  const padded = i + (f + "0".repeat(dec)).slice(0, dec);
  return new BN(padded);
}

/** Convert BigInt lamports-like to BN (for SDKs that use BN). */
export function bigIntToBN(v: bigint) {
  return new BN(v.toString(), 10);
}

/** Given a base size ladder (as human sizes for the *input mint*), produce BN ladder. */
export function makeSizeLadder(humanSizes: (string | number)[], inputMint: string) {

...
function allowedByHopCount(pathLen, lutExists) {
    if (pathLen <= CFG.maxHops)
        return true;
    if (pathLen === 3 && CFG.allowThirdHop && lutExists)
        return true;
    return false;
}
async function primeLutFromRegistry(connection, wallet) {
    ...
    for (const pool of pools) {
        try {
            ...
            console.warn('[lut] skip invalid pool id', pool.id, e?.message ?? e);
        }
    }
    if (keys.length === 0)
        return;
    try {
        ...
        console.error('[lut] prime failed', e?.message ?? e);
    }
}
async function main() {
    if (!CFG.walletSecret)
            ...
            for (const path of filteredCandidates) {
                ...
                for (let i = 0; i < path.length; i++) {
                    ...
                    try {
                        ...
                        console.warn(`[quote] ${edge.id} failed:`, e?.message ?? e);
                        quotesOk = false;
                        break;
                    }
                }
                if (!quotesOk || hopQuotes.length === 0)
                    continue;
                ...
                try {
                    ...
                    const built = await buildAndMaybeLut(sendConn, wallet.publicKey, wallet, swapIxs, 

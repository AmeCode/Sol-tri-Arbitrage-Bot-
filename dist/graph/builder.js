...
function mintForSymbol(symbol, envKey) {
    const normalized = symbol?.trim().toUpperCase();
    if (!normalized) {
        throw new Error(`[builder] token symbol missing for ${envKey}`);
    }
    const mint = MINTS[normalized];
    if (!mint) {
...
export async function buildEdges() {
    ...
    for (const p of filteredRayPools) {
        const fromMint = canonicalMint(mintForSymbol(p.a, p.key));
        const toMint = canonicalMint(mintForSymbol(p.b, p.key));
        edges.push({ ...makeRayClmmEdge(p.id, fromMint, toMint), from: fromMint, to: toMint }, { ...makeRayClmmEdge(p.id, fromMint, toMint), from: toMint, to: fromMint });
    }

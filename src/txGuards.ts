...
export function assertNoUnwhitelistedAllocations(
  instructions: TransactionInstruction[],
  payer: PublicKey,
  allowedSignerPubkeys: Set<string>,
) {
  const okSet = new Set<string>([payer.toBase58(), ...allowedSignerPubkeys]);
  ...
  for (const [idx, ix] of instructions.entries()) {
    ...
    if (ty === 'Create' || ty === 'Allocate' || ty === 'AllocateWithSeed' || ty === 'CreateWithSeed') {
      ...
      let newAcct: PublicKey | null = null;
      try {
        // Create{ newAccountPubkey }, Allocate{ accountPubkey }
        const info: any = SystemInstruction.decodeCreateAccount(ix);
        newAcct = info?.newAccountPubkey ?? null;
      } catch {
        try {
          const info: any = SystemInstruction.decodeAllocate(ix);
      ...
      if (newAcct) {
        ...
        if (!okSet.has(need) || !signerFlag) {
          ...
          throw new Error(
            `[tx-guard] SystemProgram ${ty} for account ${need} at ix#${idx} requires a signature ` +

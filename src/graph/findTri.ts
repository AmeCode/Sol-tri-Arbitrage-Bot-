...
export async function findTriCandidates(edges: PoolEdge[], seedInBase: bigint): Promise<PoolEdge[][]> {
  ...
  for (const a of edges) for (const b of edges) {
    ...
    for (const c of edges) {
      if (b.to !== c.from) continue;
      if (c.to !== a.from) continue;
      ...
      if (q3 > seedInBase) out.push([a, b, c]);
    }
  }
  return out;
}
